...
describe TutorsController do
  ...
  describe 'tutor metrics' do
    ...
    it 'expects to have 27 total hours' do
      @tutor1 = FactoryBot.create(:tutor, :id => 20, :password => 'password')
    ...
    it 'test evalutions empty' do
      @tutor1 = FactoryBot.create(:tutor, :id => 0, :password => 'password')
      expect(Tutor.average_hours_helper(@tutor1)).to eq("Not Available")
    end

    it 'average over 1 evaluation' do
      @tutor1 = FactoryBot.create(:tutor, :id => 10, :password => 'password')
